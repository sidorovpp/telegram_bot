...
import tel_consts
from tel_answers_generator import AnswersGenerator, get_json_params, parse_params
...
if not exists(tel_consts.TEL_CONFIG):
    print('Настройки не найдены')
    exit()

with open(tel_consts.TEL_CONFIG, 'r') as f:
    config = json.load(f)
...
# подключение к базе
SE = SQLExecuter(config['connect_string'])
# генератор запросов
SG = SQLGenerator()
# генератор ответов
AG = AnswersGenerator(SE, SG)
...
def check_user(user_name, chat_id):
    res = False
    if user_name:
        frame = SE.exec(SG.get_sql_text(tel_consts.TEL_START, login='@' + user_name, chat_id=chat_id))
        res = not frame.empty
    return res
    ...
    if files:
        for ff in files:
            if ff['TelegramIdent'] is not None:
                ...
                AG.exec_empty(ident=tel_consts.TEL_UPDATE_FILE,
                              _id=ff['_id'],
...
async def reply_by_ident(ident, bot, message, chat_id, login, **kwargs):
    res = AG.get_answer(ident, login=login, **kwargs)
    if type(res) == tuple:
        # один ответ
...
async def select_search(update: Update, context: tel.ContextTypes.DEFAULT_TYPE, text: str):
    items: list[list[str]] = [
        ["🔍 Клиенты", tel_consts.TEL_CLIENTS],
        ["🔍 Сотрудники", tel_consts.TEL_STAFF],
        ["🔍 Задачи", tel_consts.TEL_NEW_TASKS],
        ["🔍 Регламенты", tel_consts.TEL_DOCUMENTS],
        ["🔍 Счета", tel_consts.TEL_NEW_ACCOUNTS],
        ["🔍 Документы", tel_consts.TEL_NEW_COORDINATIONS],
    ]
...
async def document_handler(update: Update, context: tel.ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        if (len(kb) > 0) and (len(kb[0]) > 0):
            ...
            frame = AG.get_data_frame(tel_consts.TEL_FILES_FOLER)
            if frame.empty:
                return
            file_name = ''
            file_id = None
            ...
            AG.exec_empty(tel_consts.TEL_INSERT_FILE,
...
async def text_handler(update: Update, context: tel.ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        if update.message.reply_to_message is None:
            command, text = get_command(update.message.text.lower(), tel_consts.command_list)
            ...
            # обрабатываем поиск
            if command:
                ident = tel_consts.command_list[command]
...
async def callback_notifications(context: tel.CallbackContext):
    try:
        await reply_by_ident(
            get_json_params(ident=tel_consts.TEL_NOTIFICATIONS),
            context.bot,
            None,
            context.job.chat_id,
...
async def start_jobs(context: tel.CallbackContext):
    # получаем список и регистрируем job на рассылку
    frame = AG.get_answer(get_json_params(ident=tel_consts.TEL_USERS))
    for i, row in frame.iterrows():
        c = {'username': row['Login'], 'date': datetime.now()}
        context.job_queue.run_repeating(callback_notifications, interval=120, first=10, data=c, chat_id=row['Chat_id'])


application.job_queue.run_once(start_jobs, 5)

# Регистрация обработчика на текстовые сообщения, но не команды
application.add_handler(tel.MessageHandler(tel.filters.TEXT & ~tel.filters.COMMAND, text_handler))
...
application.add_handler(tel.CommandHandler(tel_consts.TEL_START, callback=start))
application.add_handler(tel.CommandHandler(tel_consts.TEL_HELP, callback=print_help))
application.add_handler(tel.CallbackQueryHandler(query_handler))
application.add_handler(tel.CommandHandler(tel_consts.TEL_NOTIFICATIONS, callback=notifications))
